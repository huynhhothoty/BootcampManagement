...
import { Modal, Divider, Radio, Table, Input, Space, Button, Row, Col, Tag, Dropdown } from 'antd';
...
const AddSubjectToSemesterModal = ({ isModalOpen, setIsModalOpen, selectedSemester, type, setIsUpdated }) => {
    ...
    const getData = () => {
        ...
        if (type === 'create'){
            semesterSubjectList.forEach((subject, index) => {
                if (allowcateFields[subject.fieldIndex].subjectList[subject.subjectIndex].isCompulsory === true)
            ...
            viewedSemesterSubjectList.forEach((subject, index) => {
                if (subject.semester === null) {
                    if (viewedAllowcatedFields[subject.fieldIndex].subjectList[subject.subjectIndex].isCompulsory === true)
                        tempSubjectList.push({
    ...
    const handleAddToBranch = (branchData) => {
        if (type === "create") {
            ...
            selectedRows.forEach((row) => {
                if (row.isGroup === false) {
                    const tempSubjectSemesterData = semesterSubjectList[row.key]
                    let newSubject = allowcateFields[tempSubjectSemesterData.fieldIndex].subjectList[tempSubjectSemesterData.subjectIndex]
                    newSubject = {
                        ...newSubject,
                        branchMajor: branchData._id
                    }

                    dispatch(editSubjectBranchMajor({
            ...
            selectedRows.forEach((row) => {
                if (row.isGroup === false) {
                    const tempSubjectSemesterData = viewedSemesterSubjectList[row.key]
                    let newSubject = viewedAllowcatedFields[tempSubjectSemesterData.fieldIndex].subjectList[tempSubjectSemesterData.subjectIndex]
                    newSubject = {
                        ...newSubject,
                        branchMajor: branchData._id
                    }
    ...
    useEffect(() => {
        setData(getData())
    }, [semesterSubjectList, viewedSemesterSubjectList, viewedAllowcatedFields, allowcateFields])
    ...
    const createDropdownBranch = () => {
        ...
        return newViewedMajor.map((branch) => {
            return {
                key: branch._id,
                label: <a onClick={() => { handleAddToBranch(branch) }}>
                    {branch.name}
                </a>
            }
        })
