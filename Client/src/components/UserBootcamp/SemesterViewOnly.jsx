  ...
  const getSemesterData = () => {
    ...
    return renderSemester.map((semester, index) => {
      ...
      semester.forEach((subject, sindex) => {
        ...
        // if(subjectData.semester === 5) console.log(subjectData)
        if (subjectData.branchMajor !== undefined) {
          if (subjectData.branchMajor !== null) {
            let belongBranchMajorIndex = viewedMajor.branchMajor.findIndex(branch => branch._id === subjectData.branchMajor)
      ...
      renderAllowcate.forEach((field, fIndex) => {
        field.electiveSubjectList.forEach((group, gIndex) => {
          if(group.branchMajor === null || group.branchMajor === undefined){
            ...
            let belongBranchMajorIndex = viewedMajor.branchMajor.findIndex(branch => branch._id === group.branchMajor._id)
            if (belongBranchMajorIndex === -1) {
              if (group.semester === index) {
                subjectList.unshift({
                  name: (() => {
              ...
              if(viewedMajor.branchMajor[belongBranchMajorIndex].isActive === false){
                ...
                if (group.semester === index) {
                  subjectBranchMajorList.unshift({
                    name: (() => {
                      if (field.isElectiveNameBaseOnBigField) {
                        let smallFieldGroupList = field.electiveSubjectList.map((ggroup, index) => {
                          return {
