      ...
      render: (_, row) => [
        <Popconfirm
          ...
          onConfirm={() => {
            ...
            dispatch(editElectiveGroupToViewedField({ fieldIndex, groupData, groupIndex: row.key }))
            setGroupCredits(0)
            setIsUpdated(true)
          }}
            ...
            type === "elective" ?
              <ProCard subTitle={groupError ? <div style={{color:"red"}}>**{VIEW_GROUP_CREDITS_NOT_EQUAL_TOTAL_CREDITS}</div> : <></>} collapsible={true} title={<h3 style={{ marginRight: 20 }} >Elective Subject Group</h3>}>
                ...
                >
                    ...
                    {({ table }) => {
                      const info = table.reduce(
                        (pre, item) => {
                          return {
                            totalSubjectCredits:
                              pre.totalSubjectCredits + item.credit
                          };
                        },
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ flex: 1 }}>Total Subject Credits: {electiveSubjectList.reduce(
                            (total, item) => {
                          ...
                          <div style={{ flex: 2, display: "flex", justifyContent: "flex-end" }}>
                            <Popconfirm
                              ...
                              onConfirm={() => {
                                ...
                                dispatch(addNewElectiveGroupToViewedField({ fieldIndex, groupData }))
                                setGroupCredits(0)
                                setIsUpdated(true)
                              }}
                              onCancel={() => setGroupCredits(0)}
                              ...
                              cancelText="Cancel"
                            >
                              <Button type='primary' disabled={totalCredits <= 0 ? true : false}>Add New Group</Button>
                            </Popconfirm>
                          </div>
                        </div>
                      );
                  ...
                  <Table columns={electiveColumns} dataSource={electiveSubjectList.map((subject, index) => ({ ...subject, index, key: index }))} />
                </ProForm>
              </ProCard>
              : <></>
          }

          <ProCard collapsible={true} title={<h3 style={{ marginRight: 20 }}>Subject List</h3>}>
            <ProForm
